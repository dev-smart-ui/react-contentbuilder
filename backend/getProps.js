...
const getProps = (html) => {
    ...
    elements.forEach((element, i) => {
        const componentType = element.getAttribute('data-component') || "type-error";
        const dataElementElements = element.querySelectorAll('[data-element]');
        const dataProps = {};

        dataElementElements.forEach((dataElement) => {
            const editableNodes = dataElement.querySelectorAll('[data-propsname]');
            editableNodes.forEach((node) => {
                const propsName = node.getAttribute('data-propsname') || "propsname-error";
                try {
                    const fields = JSON.parse(propsName);
                    Object.entries(fields).forEach(field => {
                        ...
                        const propSource = field[1];
                        dataProps[propName] = node[propSource];
                    });
                } catch (e) {
                    console.log(e);
                }
